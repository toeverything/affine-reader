...
export const getBlocksuiteReader = (config: ReaderConfig) => {
	...
	const fetchInstance = config.fetch ?? fetch
	 ...
	 * @returns
	 */
	const getDocBinary = async (docId = workspaceId, retryCD = config.retry): Promise<ArrayBuffer | null> => {
		try {
			...
			if (!response.ok) {
				...
				throw new Error(`Error getting workspace doc: ${response.status} ${response.statusText}`)
			}
	...
	if (!config.parser.buildBlobUrl) {
		config.parser.buildBlobUrl = config.blobUrlHandler ?? ((id: string) => defaultResourcesUrls.blob(target, workspaceId, id))
	}
